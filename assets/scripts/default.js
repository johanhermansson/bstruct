jQuery(function ($) {
	...
	var updatePositions = function (oldDash) {
		...
		$(".dash").each(function (i) {
			...
			if ($(this).hasClass("todo")) type = "todo";
			else if ($(this).hasClass("note")) type = "note";
			else if ($(this).hasClass("struct")) type = "struct";
		...
		$(".dash.todo .sortable")
			...
			.sortable({
				...
				sort: function (event, ui) {
					$(ui.item).parents(".dash").addClass("busy");
				},
				stop: function (event, ui) {
					$(ui.item).parents(".dash").removeClass("busy");
				},
				update: function (event, ui) {
					var dash = $(ui.item).parents(".dash"),
						item = $(ui.item),
	...
	$(".dash.todo .save").live("click", function (e) {
		...
		$.post(ROOT + "todo/item/edit", data, function (html) {
			...
			if (
				(type == "new" && value) ||
				(type == "header" && value.length > 0 && items.length < 1)
			)
				$(".add", dash).click();
		});
	});
	...
	*/
	...
	// Todo input fields
	$(".dash.todo .input").live("keyup", function (e) {
		if (e.keyCode == 13) {
	...
	$(".dash.todo .color").live("click", function () {
		...
		$.post(ROOT + "todo/todo/color", data, function (theme) {
			todo.removeClass("yellow blue green red purple").addClass(theme);
		});
	...
	$(".dash.todo .remove").live("click", function () {
		...
		if (
			confirm("This will remove the whole to do list and all attached items.")
		) {
			$.post(ROOT + "todo/todo/remove", data, function () {
				todo.hide("fold", {}, 1000, function () {
					$(this).remove();
				});
			});
		}
	...
	$(".dash.note textarea").live("keyup", function (e) {
		...
		if (number > noteMaxLength) counter.addClass("full");
		else counter.removeClass("full");
